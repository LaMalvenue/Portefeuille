...
public class SidebarUserPanel extends Panel {
	...
	private final IModel<Authentication> originalAuthenticationModel = new LoadableDetachableModel<Authentication>() {
		...
		protected Authentication load() {
			return PortefeuilleSession.get().getOriginalAuthentication();
		}
	};
			...
			new EnclosureContainer("userContainer")
				...
				.add(
					...
					ProfilePage.linkDescriptor()
						.link("profileLink")
						.add(
							new CoreLabel("fullName", BindingModel.of(PortefeuilleSession.get().getUserModel(), Bindings.user().fullName()))
								.hideIfEmpty()
						),
					new CoreLabel("username", BindingModel.of(PortefeuilleSession.get().getUserModel(), Bindings.user().username()))
						.hideIfEmpty(),
