package fr.portefeuille.web.application.common.component;
...
public class AnnouncementsPanel extends Panel {
			...
			new AjaxLink<Void>("main") {
				...
				public void onClick(AjaxRequestTarget target) {
					try {
						...
						User user = PortefeuilleSession.get().getUser();
	...
	protected void onConfigure() {
		...
		Date userLastReadDate = PortefeuilleSession.get().getUser().getAnnouncementInformation().getLastActionDate();
		...
		if (userLastReadDate == null || userLastReadDate.before(mostRecentPublicationStartDate)) {
			...
			closeModel.setObject(!PortefeuilleSession.get().getUser().getAnnouncementInformation().isOpen());
		}
	}
