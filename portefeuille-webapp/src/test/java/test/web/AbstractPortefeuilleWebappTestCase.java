...
import fr.portefeuille.core.business.history.service.IHistoryLogService;
...
import test.web.config.spring.PortefeuilleWebappTestCommonConfig;

@ContextConfiguration(classes = PortefeuilleWebappTestCommonConfig.class)
@TestPropertySource(properties = "igloo.profile=test")
public abstract class AbstractPortefeuilleWebappTestCase extends AbstractWicketTestCase<PortefeuilleWicketTester> {
	...
	@Autowired
	protected IPortefeuilleAuthenticationService authenticationService;
	...
	public void setUp() throws ServiceException, SecurityServiceException {
		...
		setWicketTester(new PortefeuilleWicketTester(application));
	}
	...
	private void initUsers() throws ServiceException, SecurityServiceException {
		basicUser = createUser("basicUser", UserTypeDescriptor.BASIC_USER,
			ImmutableSet.of(PortefeuilleAuthorityConstants.ROLE_AUTHENTICATED), ImmutableSet.of(users));
		...
		administrator = createUser("administrator", UserTypeDescriptor.TECHNICAL_USER,
			ImmutableSet.of(PortefeuilleAuthorityConstants.ROLE_ADMIN), ImmutableSet.of(administrators));
	}
